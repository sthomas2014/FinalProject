...
// kMailingList
 typedef struct
 {
    ...
    char kFifoBuff[__KERN_PAGE_SIZE];
 } kMailingList; 
...
{ 
    ...
    //Cleans up mailboxes that were previously used and closed.
...
static int ourmb_recv(const char * mailboxID, pid_t procID, char * recvBuff, int * bytesCopied)
{
    ...
    //{
        *bytesCopied = 1;
        copy_to_user ( recvBuff, mailingList[localMailboxIterator].kFifoBuff, __MAX_SIZE); 
       // haveData = 0;
    //}
